...
本结果只回答当前实现、当前 GPU 与两个固定规模下的性能趋势，不能外推为所有 stencil 尺寸、精度或架构上的普遍结论。本次没有运行 Nsight 硬件计数器；稳定的成对结果已经足以回答主问题。

后续有界探针进一步完成了性能开销归因，见
[`overhead-analysis/REPORT.md`](overhead-analysis/REPORT.md)。结果表明约 12%
的差距主要由无重叠方案覆盖同一输出域时需要 `8/7` 倍 blocks 导致，
而不是单个 block 变慢。
